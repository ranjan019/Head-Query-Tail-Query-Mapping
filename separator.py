...
def separator(file_path):
    ...
    head_tail_df = pd.DataFrame(data=None,columns=['Query','count','URL'] )
    head_list = pd.DataFrame(data=None,columns=['Query','count','URL'] )
    tail_list = pd.DataFrame(data=None,columns=['Query','count','URL'] )
    ...
    for row in log.itertuples(index=False):
        ...
        if len(pos): #for increamenting the count of the query
            ...
            ins=[row[1],row[4],1]
            head_tail_df=head_tail_df.append({'Query':row[1],'count':1,'URL':[getattr(row,"ClickURL")]},ignore_index=True)
        ind+=1
    ...
    for row in head_tail_df.itertuples(index=False):
        ...
        # pdb.set_trace()
        if row[1] < TAIL_LIMIT:
            tail_list = tail_list.append({'Query':row[0],'count':row[1],'URL':row[2]},ignore_index=True)
        elif row[1] > HEAD_LIMIT:
            head_list = head_list.append({'Query':row[0],'count':row[1],'URL':row[2]},ignore_index=True)

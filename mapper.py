...
def createHeadVectors(model):
    ...
    with open(HEAD_QUERY_PATH) as f:
        for line in f:
            head_queries.append(f.strip())
    l1 = len(head_queries)
    head_X = np.zeros((l1,word2vecLen),dtype=float)
    for i in range(l1):
        head_X[i] = wordTovec(head_queries[i],model)

    return head_queries,head_X

if __name__== "__main__":
    model = gensim.models.Word2Vec.load(Word2vec_PATH)
    tail_query = sys.argv[1]
    tail_vector = wordTovec(tail_query,model)
